...
| DEMO-04 — Bankruptcy conflict | Send attorney Monica Ferrante's dispute (dismissal order and signed authorization attached); approve the acknowledgment to counsel, record the Bankruptcy Team result in ILS and acknowledge the current handoff. See the [use-case pack](usecases/demo-4-bankruptcy-dispute/README.md). | One reviewed acknowledgment to counsel only (receipt, referral, response by October 21; no reporting outcome), then automatic reassessment and a verified transfer to Compliance; contact restrictions and unresolved concerns remain. |
