...
**Done (September 24, 2026).** Gregory P. Lindqvist (Harbor Point), represented by Monica A. Ferrante of Ferrante & Hale, LLC (`monica.ferrante@ferrantehale.example.com`). Documents: the order dismissing Chapter 13 case No. 24-10382 on July 14, 2026 with a docket excerpt (`render_court_order`, demonstration footer), the signed authorization on the firm's letterhead (`render_representation_letter`) and the internal Bankruptcy Team memo `BK-REV-260922-004` (`render_bankruptcy_memo`). **Letter decision: one acknowledgment to counsel, and nothing else** (`bankruptcy_letter`, reviewed like every Harbor Point letter). It confirms receipt, counsel-only contact, the referral and a written response by October 21, 2026, and states no reporting outcome, correction or liability. Server rules: no information request, no attachments, `acknowledgment_already_sent`, and `acknowledgment_not_sent` / `handoff_acknowledged` in `verified_outcome`; prompt `phase8-v4`. Handoffs now carry an internal `routing_note` and Compliance's `acknowledgment_note` (migration `0009`), shown in the handoff card and omitted from the model view. The diagnostic `authorized_representative` input is now Elena M. Ruiz of Brightpath Housing Counseling with a rendered *Third-Party Authorization*. Also done: `backend/tests/test_demo4_bankruptcy.py`, ILS labels, evaluation checks, docs and the pack [`usecases/demo-4-bankruptcy-dispute/`](usecases/demo-4-bankruptcy-dispute/README.md). Live: 2 API and 3 Mailbox DEMO-04 rehearsals passed, the captured Mailbox run on the final code (provider `azure_http_500` failures passed on retry). All five demos passed a Mailbox re-run on the new prompt (DEMO-05 after one provider 500), and the custom-mail evaluation passed.
